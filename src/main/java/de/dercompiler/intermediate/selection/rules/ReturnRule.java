...
public class ReturnRule extends SubstitutionRule<Return> {
    ...
    public List<Operation> substitute() {
        ...
        if (Objects.isNull(getRetVal())) {
            ...
            ret = new Ret(getRetVal());
            ret.setMode(getRetVal().getMode());
        }
        this.setTarget(null);
        return List.of(ret);
    }

    private Operand getRetVal() {
        if (getReturn().getPredCount() > 1) return getAnnotation(getReturn().getPred(1)).getTarget();
        return null;
    }
